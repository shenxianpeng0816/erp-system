...
export default function HomeScreen() {
  ...
  const { orders, isLoading, fetchMyOrders, fetchPendingOrders, fetchAllOrders } = useOrders();
  const [adminListMode, setAdminListMode] = useState<AdminOrderListMode>('pending');

  const reloadOrders = useCallback(() => {
    if (!user) return;
    if (user.role === 'ADMIN') {
      if (adminListMode === 'pending') fetchPendingOrders();
      else if (adminListMode === 'all') fetchAllOrders();
      else fetchMyOrders();
    } else if (user.role === 'SALES') {
      fetchMyOrders();
    }
  }, [user, adminListMode, fetchMyOrders, fetchPendingOrders, fetchAllOrders]);
  ...
  }, [adminListMode, user?.role, reloadOrders]);

  if (!user) return null;

  const roleModules = getRoleModules(user.role);
  const listTitle = user.role === 'ADMIN'
    ? adminListMode === 'pending' ? 'Pending Approvals'
      : adminListMode === 'all' ? 'All Orders'
      : 'My Orders'
    : 'My Orders';
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sectionRow}>
          <Text style={[styles.sectionTitle, { marginTop: user.role === 'ADMIN' ? 12 : 24 }]}>
            {listTitle}
          </Text>
        ...
        {orders.length === 0 && !isLoading ? (
          ...
          orders.map(order => (
            <OrderCard
              key={order.id}
              order={order}
              hideAmounts={hideOrderAmountsForRole(user.role)}
